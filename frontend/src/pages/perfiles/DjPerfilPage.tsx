...
export default function DjPerfilPage() {
    const { id } = useParams();
    ...
    useEffect(() => {
        const fetchDj = async () => {
            ...
            try {
                ...
                try {
                    const eventosRes = await api.get(`/eventos/dj/${id}`);
                ...
                setPagina(1); // Reinicia la página al cambiar de DJ
            } catch (error) {
                console.error("Error al cargar información del DJ:", error);
                setDj(null);
                setEventos([]);
            } finally {
                setLoading(false);
    ...
    const eventosPaginados = eventosValidos.slice((pagina - 1) * elementosPorPagina, pagina * elementosPorPagina);    return (
                ...
                <div className="container mx-auto max-w-5xl relative z-10">
                    {loading ? (
                        ...
                        <div className="text-center py-12 px-4 bg-[#121930]/50 rounded-xl border border-fuchsia-900/30 backdrop-blur-sm">
                            <h3 className="text-xl font-medium text-fuchsia-300 mb-2">DJ no encontrado</h3>
                            <p className="text-gray-400">No pudimos encontrar la información de este DJ.</p>
                        </div>
                    ) : (
                        <>
                            {/* Perfil del DJ */}
                            <div className="bg-gradient-to-br from-[#121930] to-[#1a1336] rounded-xl shadow-xl p-6 mb-10 border border-fuchsia-900/20 backdrop-blur-sm">
                                <div className="flex flex-col md:flex-row items-center gap-8">
                                    ...
                                    <div className="flex-1">
                                        <h1 className="text-3xl font-bold mb-3 neon-fuchsia font-display">{dj.nombre}</h1>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-2 mb-4">
                                            <div className="flex items-center gap-2 text-gray-200">
                                                ...
                                                <span>{dj.ubicacion || 'Sin ubicación'}</span>
                                            </div>
                                        ...
                                        {/* Valoraciones */}                                        {typeof dj.media === 'number' && (
                                            <div className="flex items-center gap-2 mb-4 bg-fuchsia-900/20 px-4 py-2 rounded-lg">
                                                <div className="flex">
                                                    {[1, 2, 3, 4, 5].map(i => (
                                                        <svg key={i} className={`w-5 h-5 ${i <= Math.round(dj.media) ? "text-fuchsia-400" : "text-gray-500"}`} fill="currentColor" viewBox="0 0 24 24"><path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"></path></svg>
                                                    ))}
                                                </div>
                                                <span className="text-sm text-fuchsia-200 font-medium">{dj.media.toFixed(1)}</span>
                                                <span className="text-xs text-gray-300">({dj.total || 0} reseñas)</span>
                                            </div>
                                        )}
                                        ...
                                        {dj.descripcion && (
                                            <div className="bg-[#0f1124]/50 rounded-lg p-4 border border-fuchsia-900/20">
                                                <p className="text-gray-300 text-sm leading-relaxed">{dj.descripcion}</p>
                                            </div>
                                        )}
                                    </div>
                                </div>
                                ...
                                {loading ? (
                                    ...
                                    <div className="text-center py-10 px-4 bg-[#121930]/50 rounded-xl border border-fuchsia-900/30 backdrop-blur-sm">
                                        ...
                                        <p className="text-gray-400">
                                            Este DJ no tiene eventos programados para las próximas fechas.
                                        </p>
                                    </div>
                                ) : (
                                    <>
                                        <div className="flex flex-col gap-8">
                                            {eventosPaginados.map((evento: any) => (
                                                <EventoCard
                                                    ...
                                                    evento={evento}
                                                    isUserLogged={!!user}
                                                    onVerDetalles={() => window.location.href = `/eventos/${evento._id}`}
