...
export default function DjsPage() {
    ...
    const fetchDjs = async () => {
        ...
        try {
            ...
            setError("No se pudieron cargar los DJs. Por favor, intenta nuevamente más tarde.");
            setDjs([]);
        } finally {
    ...
    useEffect(() => {
        setPagina(1);
    }, [nombre, ubicacion, estilo]);

    const filteredDjs = djs.filter(dj => {
        const nameMatches = !nombre || dj.nombre.toLowerCase().includes(nombre.toLowerCase());
        const locationMatches = !ubicacion || (dj.ubicacion || "").toLowerCase().includes(ubicacion.toLowerCase());
        let genresMatch = !estilo;
        
        if (estilo) {
            const djGenres = dj.estilosMusicales || dj.gustosMusicales || [];
            if (djGenres.length === 0) {
                genresMatch = false;
            } else {
                const estiloLowerCase = estilo.toLowerCase();
                genresMatch = djGenres.some((g: string) => {
                    const match = g.toLowerCase().includes(estiloLowerCase) || estiloLowerCase.includes(g.toLowerCase());
                    return match;
                });
            }
        }
        
        return nameMatches && locationMatches && genresMatch;
    });

    const totalPaginas = Math.ceil(filteredDjs.length / elementosPorPagina);
    const djsPaginados = filteredDjs.slice((pagina - 1) * elementosPorPagina, pagina * elementosPorPagina);
            ...
            <div className="relative py-16 px-4 overflow-hidden">
                ...
                <div className="relative z-10 container mx-auto max-w-5xl">
                    ...
                    <p className="text-center text-gray-300 max-w-2xl mx-auto mb-10">
                        Descubre los mejores DJs y explora su música
                    </p>
                    <div className="mb-12">
                    ...
                    <div className="flex flex-col gap-8 min-h-[300px]">
                        {loading ? (
                            <div className="flex flex-col items-center justify-center py-20 min-h-[400px]">
                                ...
                                <p className="text-fuchsia-300 font-medium text-lg tracking-wide animate-pulse">Cargando DJs...</p>
                            </div>
                        ) : (
                            <>
                                {djsPaginados.map(dj => (
                                    <div key={dj._id} onClick={() => navigate(`/djs/${dj._id}`)}>
                                        <DjCard dj={dj} />
                                    </div>
                                ))}
                                {filteredDjs.length === 0 && !loading && (
                                    <div className="text-center py-12 px-4 bg-[#121930]/50 rounded-xl border border-fuchsia-900/30 backdrop-blur-sm">
                                        ...
                                        <p className="text-gray-400">
                                            No encontramos DJs que coincidan con tu búsqueda. Prueba con otros filtros.
                                        </p>
                                    </div>
                                )}
                            </>
